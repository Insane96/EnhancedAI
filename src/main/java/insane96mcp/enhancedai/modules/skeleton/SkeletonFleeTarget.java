...
public class SkeletonFleeTarget extends Feature {
    ...
    public static void onReassessWeaponGoal(AbstractSkeleton skeleton) {
        ...
        boolean avoidTarget = NBTUtils.getBooleanOrPutDefaultLegacy(persistentData, EATags.Flee.AVOID_TARGET, skeleton.getRandom().nextDouble() < avoidPlayerChance);
        if (!avoidTarget)
            return;

        boolean attackWhenAvoiding = NBTUtils.getBooleanOrPutDefaultLegacy(persistentData, EATags.Flee.ATTACK_WHEN_AVOIDING, skeleton.getRandom().nextDouble() < attackWhenAvoidingChance);
