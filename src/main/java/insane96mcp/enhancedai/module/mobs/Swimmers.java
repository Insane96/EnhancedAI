...
@LoadFeature(module = EAIModules.MOBS, description = "Makes drowned swim speed based off swim speed attribute instead of movement speed.")
public class Swimmers extends Feature {
	...
	final ResourceLocation SWIM_SPEED_MULTIPLIER_ID = EnhancedAI.location("drowned_swim_speed_multiplier");
	...
	public void onEntityJoinLevel(EntityJoinLevelEvent event) {
        ...
        if (drowned$swimSpeedMultiplier > 0d && event.getEntity() instanceof LivingEntity livingEntity && livingEntity.getType().is(SWIM_SPEED_MULTIPLIER_AFFECTED))
            MCUtils.applyModifier(livingEntity, NeoForgeMod.SWIM_SPEED, SWIM_SPEED_MULTIPLIER_ID, drowned$swimSpeedMultiplier - 1, AttributeModifier.Operation.ADD_MULTIPLIED_TOTAL);
