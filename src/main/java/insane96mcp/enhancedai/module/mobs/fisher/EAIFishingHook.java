...
public class EAIFishingHook extends Projectile {
    private static final EntityDataAccessor<Integer> DATA_HOOKED_ENTITY = SynchedEntityData.defineId(EAIFishingHook.class, EntityDataSerializers.INT);
    private int life;
    @Nullable
    private Entity hookedIn;
    private FishHookState currentState = FishHookState.FLYING;

    public EAIFishingHook(EntityType<? extends EAIFishingHook> p_150141_, Level p_150142_) {
        super(p_150141_, p_150142_);
        this.noCulling = true;
    }

    public EAIFishingHook(Entity p_37106_, Level p_37107_) {
    ...
    private void checkCollision() {
        ...
        if (hitresult.getType() == HitResult.Type.MISS || !EventHooks.onProjectileImpact(this, hitresult)) this.onHit(hitresult);
    }
    ...
    public void retrieve(boolean isInventoryHooked) {
        ...
        if (this.hookedIn != null) {
            if (this.hookedIn instanceof LivingEntity living && isInventoryHooked) {
                ItemStack itemStack;
                EquipmentSlot slot = this.random.nextBoolean() ? EquipmentSlot.MAINHAND : EquipmentSlot.OFFHAND;
